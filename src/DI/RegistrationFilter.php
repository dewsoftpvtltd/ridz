...
class RegistrationFilter
{
    ...
    {
      $required = new RequiredFilter();
      if (isset($this->values['username'])){
        $required->setValue($this->values['username']);
      }
        ...
        if(preg_match('/[^a-z\d_-]/i',$this->values['username'])){
          $valid = false;
          $this->addMessage('username', 'Username contains invalid characters');
        }

        $this->values['username'] = strtolower($this->getValue('username'));
        return $valid;
    }
